package com.example.androidplatform.presentation.confirm_refill_operation.viewmodel
...
class ConfirmRefillOperationViewModel (
    ...
    fun refill(recipientAccountNumber: String, sum: String) {
        viewModelScope.launch(Dispatchers.IO) {
            val launchOperationResult = launchOperationInteractor.launchOperation(OperationCode.ACCOUNT_REFILL)
            launchOperationResult.collect { data ->
                handleLaunchOperationResult(data, recipientAccountNumber, sum)
            }
        }
    }

    private suspend fun handleLaunchOperationResult(data: SearchResultData<OperationItem>, recipientAccountNumber: String, sum: String) {
        when (data) {
            is SearchResultData.Data -> proceedOperation(data.value!!.requestId, recipientAccountNumber, sum)
            else -> handleError(data)
        }
    }

    private suspend fun proceedOperation(requestId: Int, recipientAccountNumber: String, sum: String) {
        val proceedOperationResult = proceedOperationInteractor.proceedOperation(
            requestId,
            arrayListOf(
                ProceedOperationRequestItem(ACCOUNT, "[$recipientAccountNumber]"),
